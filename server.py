...
import cv2, pyaudio, wave, time, numpy as np
...
ALLOWED_EXTENSIONS = {'ogg','wav','mp3','mpeg'}
...
def uploads():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            wf_name = file.save(f'uploads/{filename}')
            data, samplerate = sf.read(f'uploads/{filename}')
            sf.write('uploads/msg.wav',data,16000)
            wf = wave.open(f'uploads/{filename}')
            stream = audio.open(format=audio.get_format_from_width(wf.getsampwidth()),
                                channels=wf.getchannels(),rate=wf.getframerate(),output=True)
